package aquadrop_latam.listener;
...
import aquadrop_latam.events.PaymentAuthorizedEvent;
import aquadrop_latam.events.PaymentFailedEvent;
import aquadrop_latam.events.TankerAssignedEvent;
        ...
        try{
            ...
            switch (eventType) {
                ...
                case "TankerAssignedEvent":
                    ...
                    if(tankerEvent.status().equals("TANKER_ASSIGNED")) {
                        handleTankerAssigned(tankerEvent);
                    } else if(tankerEvent.status().equals("TANKER_REJECTED")) {
                        handleTankerRejected(tankerEvent);
                    }
                    break;
                default:
                    System.out.println("Evento no reconocido: " + eventType);
            }
        } catch(Exception e) {
            e.printStackTrace();
        }
